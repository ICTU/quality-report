...
class GreenMetaMetric(MetaMetricMixin, HigherPercentageIsBetterMetric):
    ...
    norm_template = 'Minimaal {target}% van de metrieken scoort groen ' \
        '(op of boven de norm). Minder dan {low_target}% is rood.'
    template = '{value}% van de metrieken ({numerator} van de ' \
        '{denominator}) scoort boven de norm.'
    target_value = 90
    low_target_value = 80
...
class RedMetaMetric(MetaMetricMixin, LowerPercentageIsBetterMetric):
    ...
    metric_statuses = ('red',)
    norm_template = 'Maximaal {target}% van de metrieken scoort rood ' \
        '(direct actie vereist). Meer dan {low_target}% is rood.'
    template = '{value}% van de metrieken ({numerator} van de ' \
        '{denominator}) scoort rood.'
    target_value = 2
    low_target_value = 5
...
class YellowMetaMetric(MetaMetricMixin, LowerPercentageIsBetterMetric):
    ...
    norm_template = 'Maximaal {target}% van de metrieken scoort geel ' \
        '(onder de norm maar niet direct actie vereist). ' \
        'Meer dan {low_target}% is rood.'
    template = '{value}% van de metrieken ({numerator} van de ' \
        '{denominator}) scoort geel.'
    target_value = 5
    low_target_value = 10
...
class GreyMetaMetric(MetaMetricMixin, LowerPercentageIsBetterMetric):
    ...
    norm_template = 'Maximaal {target}% van de metrieken scoort grijs ' \
        '(geaccepteerde technische schuld). Meer dan {low_target}% is rood.'
    template = '{value}% van de metrieken ({numerator} van de ' \
