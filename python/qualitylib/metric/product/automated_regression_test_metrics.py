...
from ..quality_attributes import TEST_COVERAGE, PERFORMANCE, TEST_QUALITY
...
class FailingRegressionTests(LowerIsBetterMetric):
    ...
    def value(self):
        if self._missing():
            ...
            return self.__test_report.failed_tests(*self.__report_urls()) + \
                   self.__test_report.skipped_tests(*self.__report_urls())

    def _missing(self):
        return self.__test_report.passed_tests(*self.__report_urls()) < 0 or \
               self.__test_report.failed_tests(*self.__report_urls()) < 0 or \
               self.__test_report.skipped_tests(*self.__report_urls()) < 0
    ...
    def __report_urls(self):
        ...
        return [report_urls] if type(report_urls) == type('') else report_urls
...
class _ARTCoverage(HigherIsBetterMetric):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self._subject.product_version():
            ...
            self.norm_template = 'Minimaal {target}% van de {covered_items} ' \
...
class ARTStatementCoverage(_ARTCoverage):
    # pylint: disable=too-many-public-methods
    ''' Metric for measuring the statement coverage of automated regression tests (ART)
        for a product. '''

    name = 'Automatic regression test statement coverage'
    norm_template = 'Minimaal {target}% van de statements wordt gedekt ' \
...
class ARTBranchCoverage(_ARTCoverage):
    # pylint: disable=too-many-public-methods
    ''' Metric for measuring the branch coverage of automated regression tests (ART)
        for a product. '''

    name = 'Automatic regression test branch coverage'
    norm_template = 'Minimaal {target}% van de branches wordt gedekt ' \
           'door geautomatiseerde functionele tests. ' \
           'Minder dan {low_target}% is rood.'
