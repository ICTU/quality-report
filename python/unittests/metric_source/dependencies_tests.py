...
class FakeFile(object):  # pylint: disable=too-few-public-methods
    ''' Provide a fake file object so we don't need to access to the file 
        system. '''
...
class DependenciesTest(unittest.TestCase):
    ...
    def test_set_and_get_dependencies(self):
        ''' Test that the dependencies for a product can be put into the 
            database. '''
        self.__dependencies.set_dependencies('name', 'version', 1)
        self.assertEqual(1, self.__dependencies.get_dependencies('name', 
                                                                 'version'))
