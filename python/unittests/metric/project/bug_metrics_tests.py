...
class TechnicalDebtIssuesTest(unittest.TestCase):
    ...
    def test_value(self):
        ''' Test that the value is correct. '''
        self.assertEqual(FakeJira.nr_technical_debt_issues(),
                         self.__metric.value())

    def test_url(self):
        ''' Test that the url is correct. '''
        self.assertEqual({'Jira': FakeJira.nr_technical_debt_issues_url()},
                         self.__metric.url())
    ...
    def test_cant_be_measured_without_technical_debt_issues_query(self):
        ''' Test that the metric cannot be measured without a technical debt issues query
            in Jira. '''
        jira = FakeJira(has_queries=False)
        project = domain.Project(metric_sources={metric_source.Jira: jira})
        self.assertFalse(metric.TechnicalDebtIssues.can_be_measured(self.__project, project))
