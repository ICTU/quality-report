...
class ReadyUserStoryPointsTest(unittest.TestCase):
    ...
    def test_url(self):
        ...
        self.assertEqual({'Jira': FakeJira.user_stories_ready_url()},
                         self.__metric.url())

    def test_can_be_measured(self):
        ''' Test that the metric can be measured when the project has Jira,
            and Jira has an ready user stories query. '''
        self.assertTrue(metric.ReadyUserStoryPoints.can_be_measured(self.__project,
                                                                    self.__project))
    ...
    def test_cant_be_measured_without_ready_user_stories_query(self):
        ''' Test that the metric cannot be measured without an ready user stories query
            in Jira. '''
