...
class JavaVersionConsistencyTests(unittest.TestCase):
    ...
    def setUp(self):  # pylint: disable=invalid-name
        ...
        self._project = domain.Project(metric_sources={metric_source.AnsibleConfigReport:
                                                       self.__report},
                                       metric_source_ids={self.__report: 'http://ansible_report'})
        self._metric = metric.JavaVersionConsistency(subject=self._project,
                                                     project=self._project)

    def test_can_be_measured(self):
        ''' Test that the metric can be measured if there is a config report. '''
        self.assertTrue(metric.JavaVersionConsistency.
    ...
    def test_value(self):
        ...
        self.assertEqual(2, self._metric.value())

    def test_url(self):
        ''' Test that the url of the metric equals the url of Jenkins. '''
        self.assertEqual({'Ansible configuration report': 'http://ansible_report'},
                         self._metric.url())

    def test_report(self):
        ''' Test the metric report. '''
        self.assertEqual('Er zijn 2 verschillende versies van Java in gebruik.', self._metric.report())
